...
\section{Begrep}
\textbf{Punkt}\os
En bestemt plassering kalles et\footnote{Se også \hrs{Koord}{seksjon}} \textit{punkt}\index{punkt}. Et punkt markerer vi ved å tegne ein prikk, som vi gjerne setter navn på med en bokstav. Under har vi tegnet punktene $ A $ og $ B $.
\fig{punkt1}
...
\fig{linj1}
En rett strek som går mellom to punkt kallar vi et \textit{linjestykke}\index{linjestykke}.
\fig{linjstk1}
Linjestykket mellom punktene $ A $ og $ B $ skriv vi som $ AB $. 

\info{Merk}{
	Et linjestykke er et utklipp (et stykke) av ei linje, derfor har ei linje og et linjestykke mange felles egenskaper. Når vi skriver om linjer, vil det bli opp til leseren å avgjere om det samme gjelder for linjestykker, slik sparer vi oss for hele tiden å skrive ''linjer/linjestykker''.
}
\newpage
...
\begin{itemize}
\item Til hvilken tid man snakkar om et linjestykke og hvilken tid man snakkar om en lengde vil komme tydeleg frem av sammenhengen begrepet blir brukt i.
\item Å hele tiden måtte ha skrevet ''lengda til $ AB $'' o.l. ville gitt mindre leservennlige setninger.
\end{itemize}
}
\newpage
\textbf{Avstand}\os
...
\fig{parl1}
Vi bruker symbolet \sym{$ \parallel $} for å vise til at to geometrier er parallelle.
\[ AB\parallel CD \]
\fig{parl1a}
\textbf{Vinklar} \label{vinklar}\os
To linjer som ikke er parallelle, vil før eller siden krysse hverandre. Gapet to linjer danner seg i mellom kalles en \textit{vinkel}\index{vinkel}. Vinkler tegner vi som små sirkelbuer:
\fig{vink1}
Linjene som dannar en vinkel kaller vi \textit{vinkelbein}\index{vinkelbein}. Punktet der linjene møtes kaller vi \textit{toppunktet}\index{vinkel!toppunkt til} til vinkelen. Ofte bruker vi punktnavn og vinkelsymbolet \sym{$ \angle $} for å gjøre tydelig hvilken vinkel vi meiner. I figuren under er det slik at
\begin{itemize}
...
\info{Kva vinkel?}{
	...
	\fig{vink2a}
	Så lenge det bare er to linjestykker/linjer som danner en vinkel, er det også vanlig å bruke bare én bokstav for å vise til vinkelen:
	\fig{vink2b}
}\vsk
\label{vinkelend}
\newpage
...
\fig{kant2}
\info{Merk}{
 Ofte kommer vi til å skrive bare en bokstav for å markere et hjørne i ein mangekant.
\fig{kant2b}
} \vsk
...
\fig{tri15b}
\info{Merk}{Høgde og grunnlinje kan også på liknende vis bli brukt i samband med andre mangekantar.}
...
\eks{
Da en likesidet trekant har tre sider som er like lange og tre vinkler som er $ 60^\circ $, er den også ein likebeint trekant.
}
\spr{
...
\eks{
Kvadratet er både en rombe og et rektangel, og ''arvar'' derfor egenskapane til disse. Dette betyr at i et kvadratet er
\begin{itemize}
...
\reg[\firsum \label{360}]{I ein firkant er summen av vinkelverdiene $ 360^\circ $.
...
\st{Vi tenker oss et kvadrat med sidelengder 1. Dette kallar vi \\\textit{enerkvadradet}.
	\fig{tri_10}
...
\fig{tri11a}
Vi kan da telle oss frem til at rektangelet har plass til 6 enerkvadrat:
...
\info{Breidde og lengde}{Ofte blir ordene \textit{bredde}\index{bredde} og \textit{lengde}\index{lengde} brukt om grunnlinja og høgda i et rektangel.}
\eks[1]{
