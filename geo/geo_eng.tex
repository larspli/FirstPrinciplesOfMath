...
\section{\omgr}
\textbf{Punkt}\os
A given position is called a\footnote{See also \refsec{Koord}.} \textit{point}\index{point}. We mark a point by drawing a dot, which we preferably name by a letter. Beneath we have drawn the points $ A $ og $ B $.
\fig{punkt1}
\textbf{Line and segment}\os
A straight dash with infinite length (!) is called a \textit{line}\index{line}. The fact that the line has infinite length makes \textsl{drawing} a line impossible, we can only \textsl{imagine} a line. Imagining a line we can do by drawing a straight dash and think of its ends as wandering out in each direction.
\fig{linj1}
A straight dash between two points is called a \textit{segment}\index{segment}.
\fig{linjstk1}
The segment between the points $ A $ and $ B $ we write as $ AB $.  
\info{Notice}{
A segment is an excerpt of a line, therefore a line and a segment have a lot of attributes in common. When writing about lines, it will be up to the reader to confirm whether the same applies for segments. Hence we avoid the need of writing ''lines/segments''.
}
...
\fig{vink1}
Lines creating an angle are called the \textit{sides}\index{angle!side} of the angle. The intersection point of the lines are called the  \textit{vertex}\index{vinkel!vertex} of the angle. It is common to use the symbol \sym{$ \angle $} to underline the angle in question. In the figure below we have the following:
\begin{itemize}
...
When measuring an angle in degrees, we imagine a circular curve divided into 360 equally long pieces. We call one such piece one \textit{degree}\index{degree}, indicated by the symbol \sym{$ ^\circ $}. 
\fig{vink3} \vsk
Notice that an angle with measure $ 90^\circ $ is indicated by the symbol \sym{$ \square $}. Such an angle is called a \textit{right}\index{angle!right} angle. Lines/segments which form right angles are said to be \textit{perpendicular}\index{segment!perpendicular} to one another, and this we indicate by the symbol $ \sym{$ \perp $} $.
\[ AB\perp CD \]
\fig{vink3a}
\newpage
\info{What angle?}{
	Strictly speaking, when two segments (or lines) intersect they form two angles; the one larger than $ 180^\circ $ and the other smaller than $ 180^\circ $. Usually it is the smaller angle we wish to study, therefore it is common to define $ \angle AOB $ as the \textsl{smaller} angle formed by the segments $ OA $ og $ OB $.
	\fig{vink2a}
	As long as there are only two segments/lines present, it is also common using only one letter to indicate the angle:
	\fig{vink2b}
}\vsk
\label{vinkelend}
\newpage
...
When line segments form an enclosed shape, we have a \textit{polygon}\index{polygon}. The figure below shos (from left to right) a triangle\index{triangle}, a quadrilateral\index{quadrilateral} and a pentagon.
\fig{kant1}
The segments of a polygon are called \textit{edges}\index{edge} or \textit{sides}\index{side!of polygon}. The respective intersection points of the segments are the \textit{vertices}\index{polygon!vertices of} of the polygon. That is, the triangle below have vertices $ A $, $ B $ and $ C $ and sides (edges) $ AB $, $ BC $ og $ AC $.
\fig{kant2}
\info{Noitce}{
Often we'll only write a letter to indicate a vertex of a polygon.
\fig{kant2b}
} \vsk

\textbf{Diagonals} \os
Segments between two vertices not belonging to the same side of a polygon is called a \textit{diagonal}. The figure below shows the diagonals $ AC $ and $ BD $.
\fig{kant7_eng}
\newpage
\subsubsection{Altitudes and base lines}
In \refsec{Areal}, the terms \textit{base}\index{base} and \textit{height}\index{height} (\textit{altitude}\index{altitude}) plays an important role. To find the height of a triangle, we choose one of the sides to be the base. In the figure below, let's start with $ AB $ as the base. Then the height is the segmet from $ AB $  (potentially, as is the case here, the extension of $ AB $) to $ C $, perpendicular to $ AB $.
\fig{tri15_eng}
...
\spr{
	The longest side of a right triangle is called the \textit{hypotenuse}\index{hypotenuse}. The shortest sides are called \textit{legs}\index{legs}.
}
...
\reg[Quadrilaterals \label{firkantar}]{
\fig{kant3g_eng}
\begin{figure}
...
\section{\omkr}
When we measure the length around an enclosed shape, we fint its \textit{perimeter}\index{perimeter}. Let's find the perimeter of this rectangle:
\fig{geo1}
The rectangle have two sides of length 4 and two sides of length 5.
\fig{geo1a}
Hence
\alg{
...
\reg[Perimeter]{Perimeter is the length around a closed shape.}
\eks[]{ \vsb \vs
	\begin{figure}
		\centering
...
Our surroundings are full of \textit{surfaces}\index{surface}, for example the on a floor or a sheet. When measuring surfaces, we find their \textit{area}\index{area}. The concept of area is the following:\regv

\st{We imagine a square with sides of length 1. We call this the \textit{one-square}.
	\fig{tri_10}
	Then, regarding the surface for which we seek the area of, we ask:\os
	\begin{center}
		''How many one-squares does this surface contain?''
\end{center}}
\subsubsection{\arrekt \label{arrekt}}
Let's fin the area of a rectangle with baseline 3 and altitude 2.
\fig{tri11a}
Simply by counting we find that the rectangle contains 6 one-squares:
...
\newpage
\reg[The area of a rectangle \label{arfir}]{
\vs
...
\footnotetext{\mer The lengths used in one figure will not necessarily correspond with the lengths in another figure. That is, a side of length 1 in one figure can might as well be shorter than a side of length 1 in a another figure.}
\newpage
...
\fig{tri16}
We can make a rectangle by copying our triangle, then joining the sides which are not the heights and altitudes in question:
\fig{tri17}
...
\[\frac{5\cdot3}{2}= \frac{\text{baseline}\cdot \text{altitude}}{2} \]
\newpage
\textit{2) The altitude is placed outside the triangle} \os
The triangle below has baseline 4 and altitude 3. 
\fig{tri18}
We make a rectangle containing the blue triangle:
\fig{tri18a}
...
We have (both the red and the green triangles are right-angled)
\alg{
	R&= 3\cdot10=30\vn
...
The triangle below has baseline 5 and altitude 4.
\fig{tri20}
We make a rectangle containing the blue triangle (split into red and yellow triangles):
\fig{tri20a}
Observe that
\begin{itemize}
...
\end{itemize}
It now follows that the sum of the areas of the yellow and the red triangle makes up half the area of the rectangle formed by the four colored triangles. The area of this rectangle equals $ 5\cdot4 $, and since our original triangle (the blue) includes the red and the orange triangle, we have
\[ \text{The areae of the blue triangle}=\frac{5\cdot4}{2}=\frac{\text{baseline}\cdot\text{altitude}}{2} \] 
\newpage
\textit{All three cases summarized}\os
One of the cases discussed will always be valid for a chosen baseline in a triangle. All cases resulted in the same expression for the area of the triangle.\regv

\reg[Arealet til ein trekant]{
	\[ \text{Areal}=\frac{\text{base}\cdot\text{height}}{2} \]
\fig{triar00_eng}
}
\eks[1]{
