...
\info{Notice}{
A segment is a cut of a line, therefore a line and a segment have a lot of attributes in common. When writing about lines, it will be up to the reader to confirm whether the same applies for segments. Hence we avoid the need of writing ''lines/segments''.
}
...
\fig{vink1}
Lines creating an angle are called the \textit{sides}\index{angle!side} of the angle. The intersection point of the lines are called the  \textit{vertex}\index{vinkel!vertex} of the angle. It is common to use the symbol \sym{$ \angle $} to underline the angle in question. In the below figure we have the following:
\begin{itemize}
