...
Skulle vi jobbet videre med funskjonen vi akkurat har sett på, ville det blitt tungvint å heile tiden måtte skrive ''Antall ruter i \textsl{Figur }$ x $''. Det er vanleg å kalle også funksjoner bare for en bokstav, og i tillegg skrive variabelen funskjonen er avhengig av i parantes. La oss nå omdøpe funksjonen ''Antal ruter i \textsl{Figur} $ x $'' til $ a(x) $. Da har vi at
\[ \text{Antall ruter i \textsl{Figur }}x=a(x)=2x+1 \]
Hvis vi skriver $ a(x) $, men erstatter $ x $ med et bestemt tall, betyr det at vi skal erstatte $ x $ med dette talet i funksjonsuttrykket vårt:
\alg{
...
\reg[\funklin \label{funklin}]{
	En funksjon på forma \vs
	\[ f(x)=ax+b \]
	er en \textit{lineær} funksjon med \textit{stigningstall}\index{stigningstall} $ a $ og \textit{konstantledd}\index{konstantledd} $ b $. \vsk 
...
\newpage
\eks[1]{Finn stigningstaletl og konstantleddet til funksjonene.
	\alg{
		f(x)&=2x+1 \vn
		g(x)&=-3+\frac{7}{2}	\vn
...
\eks[2]{
	...
	\sv
	For å tegne grafen til ein lineær funksjon trenger vi bare å finne to punkt som ligger på grafen. Hvilke to punkt dette er, er det fritt å velge, så for enklest mulig utregning starter vi med å finne punktet der $ x=0 $:
...
Gitt ein lineær funksjon $ f(x)=ax+b $ og to forskjellige $ x $-verdiar $ x_1 $ og $ x_2 $. I figuren som viser grafen til $ f(x) $ let vi $ {A=(x_1, b)} $, $ B=(x_2, b) $, $ C=(b, f(x_1)) $, $ D=(0, f(x_2)) $ og $ E=(0, b) $.
\fig{funk6a}
