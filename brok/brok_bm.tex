...
\reg[\brdef \label{brdef}]{
	En brøk\index{brøk} er en annan måte å skrive et delestykke på. I en brøk kaller vi dividenden for \textit{teller}\index{teller} og divisoren for \textit{nevner}\index{nevner}.
	\begin{figure}
		\centering
...
\fig{br3}
Slik vil de blå og de tomme rutene fortelle oss hvor mange biter 1 er delt inn i, mens de blå rutene alene forteller oss hvor mange slike bitar det \textsl{egentlig} er. Slik kan vi seie at\regv
...
\begin{itemize}
	...
	\item Telleren forteller kor mange slike biter det er.
\end{itemize}
\newpage
\section{\brvu}
...
\subsubsection{Brøker med same verdi}
...
At brøker kan se forskjellige ut, men ha samme verdi betyr at vi kan endre på utseendet til en brøk uten å endre verdien. La oss som eksempel gjøre om $ \frac{3}{5} $ til en brøk med same verdi, men med 10 som nevner:
\begin{itemize}
	\item $ \frac{3}{5} $ kan vi gjøre om til en brøk med 10 i nevner om vi deler hver femdel inn i 2 like biter, for da blir 1 til sammen delt inn i\\ $ {5\cdot2=10} $ bitar.
	\item Telleren i $ \frac{3}{5} $ forteller at der er 3 femdeler. Når disse 3 delene blir delt i to, blir de totalt til $ 3\cdot2=6 $ tidelar. Altså har $ \frac{3}{5} $ samme verdi som $ \frac{6}{10} $.
\end{itemize}
...
\eks[3]{
	...
	\sv
	Da $ 30:6=5 $, deler vi både teller og nevner med 5:
	\alg{
...
\fig{br7}
\fig{br7t}
Skal vi skrive differansen som en brøk, må vi sørge for at brøkene har samme nevner. De to brøkane våre kan begge ha 10 som nevner:
\alg{
...
\fig{br7a}
\fig{br7ta}
Det vi har gjort, er å utvide begge brøkene slik at de har same nevner, nemlig 10. Når nevnerne i brøkane er like, kan vi regne ut subtraksjonsstykket mellom tellerene:
\alg{
...
\[ \frac{7}{6}+\frac{5}{3} \]
Her kan vi bruke fellesnevneren $ {6\cdot3=18} $, men det er bedre å merke seg at $ 6\cdot1=3\cdot2=6 $ også er en fellesnevner.
}
\newpage
\eks[2]{
	Regn ut
...
Av \rref{brhel} kan vi også danne en tolkning av hva å gange med ein brøk innebærer. For eksempel, å gange $ 3 $ med $ \frac{2}{5} $ kan tolkes på disse to måtene:
\begin{itemize}
...
\fig{br15}
\fig{br15t}
Vi har her 4 åttedeler som vi skal fordele likt på 2. Dette blir $ 4:2=2 $ åttedeler.
\fig{br15a}
\fig{br15ta}
Altså er
\[ \frac{4}{8}:2=\frac{2}{8} \]
...
De to svarene har altså samme verdi. Saken er at skal vi dele en brøk på et heltall, og telleren er delelig med heitalet, kan vi direkte dele telleren på heltallet. I slike tilfeller er det altså ikke feil, men heller ikke nødvendig å bruke \rref{brdeenk}.
}
...
Med tolkningen akkurat nemnt, skal vi nå gange $ \frac{5}{4} $ først med 3, og så dele produktet med 2. Av \rref{brhel} er
\alg{
...
\newpage
\info{Brøker forenklar utregninger}{
	Desmialtallet $ 0,125 $ kan vi skrive som brøken $ \frac{1}{8} $. Reknestykket
	\[ 0,125\cdot16 \]
	vil for de fleste av oss ta en stund å løse for hand med vanlige mulitplikasjonsregler. Men bruker vi brøkuttrykket får vi at
	\alg{
...
\info{''Å stryke nuller''}{
	...
	I praksis er dette det same som ''å stryke nuller'':
	\alg{
...
Vi kan ikke se på en tallinje hver gang vi skal dele med brøker, så nå skal vi komme frem til en generell reknemetode ved igjen å bruke $ 4:\frac{2}{3} $ som eksempel. For denne metoden bruker vi denne betydningen av divisjon:
\begin{center}
	$  4:\dfrac{2}{3}= $ ''Tallet vi må gange $ \frac{2}{3} $ med for å få 4.''
\end{center}
For å finne dette talet starter vi med å gange $ \frac{2}{3} $ med tallet som gjør at produktet blir 1. Dette tallet er \textit{den omvendte brøken}\index{brøk!omvendt} av $ \frac{2}{3} $, som er $ \frac{3}{2} $:
