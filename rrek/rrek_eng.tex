...
\sv \vs \vs
\algv{
&& 23-(3+9)+4\cdot 7&=23-12+4\cdot7 &&\text{Parantheses} \\
...
\eks[2]{
	...
	\sv \vs \vs
	\algv{
		&& 18:(7-5)&=18:2-3 &&\text{Parantheses} \\
		&&&=9-3 &&\text{Division} \\
		&&&=6 &&\text{Addition and subtraction}
	}
}
\subsection*{Multiplication involving paranthesis}
How many boxes are present in this figure?
\fig{gang}
To correct interpretations include:
\begin{enumerate}
...
\info{\merk}{
We introduced parenthesis as an indicator of what to calculate first, but \rref{gangpar} gives an alternative and equivalent interpretation of parenthesis . The rule is especially useful when working with algebra (see \hrs{Del2}{Part}\,).
}
\newpage
...
The placement of parentheses between terms have no impact on the sum.
}
\eks[]{ \vsb \vs
\alg{
(2+3)+4&=8 \vn
...
The placement of parentheses between factors have no impact on the product.
}
\eks[]{ \vsb \vs
\alg{
