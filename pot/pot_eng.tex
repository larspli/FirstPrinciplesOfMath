...
\spr{
Common ways of saying $ 2^3 $ are\footnote{Attention! The examples illustrates a small paradox of the English language. Thing is, \textit{power} is also a (and in the spoken language the preferred) synonym for \textit{exponent}.}
\begin{itemize}
	\item ''2 to the power of 3''
	\item ''2 to the third power''
\end{itemize}

In programming languages one usually writes the symbol \sym{\^{}} or the symbols \sym{**} between the base and the exponent.
}
\newpage
\info{Notice}{
The next pages declares rules concerning powers with corresponding explanations. Even though one wish to have these explanations as general as possible, we choose to use exponents which are not variables. Using variables as exponents would lead to less reader-friendly expressions and it is our claim that the general cases are well illustrated by the specific cases. 
} \vsk \vsk
...
\eks[2]{ \vs \vsb
\alg{
\frac{2^{4}\cdot a^{3}}{a^{2}\cdot2^{2}}&=2^{4-2}\cdot a^{3-2}\\
&=2^{2}a \\
&=4a
}
}
...
\fork{\ref{potbr} \potbr}{
	...
	We have
	\begin{align*}
	\left(\frac{a}{b}\right)^3 & =\frac{a\cdot a\cdot a}{b\cdot b\cdot b}\br
	& =\frac{a^{3}}{b^{3}}
	\end{align*}
}\vsk \vsk
...
\fork{\ref{potsomgrunn} \potsomgrunn}{
	Let's use $\left(a^{3}\right)^{4}$ as an expample. We have
	\begin{align*}
...
\spr{
$\sqrt{9} $ is called '' the square root of 9'' \vsk

$\sqrt[3]{8} $ is called ''the cube root of 8'' \vsk

$ \sqrt[5]{9} $ is called ''the 5th root of 9''.
}
\newpage
\section{\irrasj}
